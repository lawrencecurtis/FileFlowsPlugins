...
public class FfmpegBuilderExecutor: FfmpegBuilderNode
{
    ...
    public override int Execute(NodeParameters args)
    {
        ...
        foreach (var item in model.VideoStreams.Select((x, index) => (stream: (FfmpegStream)x, index, type: 1, list: model.VideoStreams.Select(x => (FfmpegStream)x).ToList())).Union(
                             ...
                             model.SubtitleStreams.Select((x, index) => (stream: (FfmpegStream)x, index, type: 3, list: model.SubtitleStreams.Select(x => (FfmpegStream)x).ToList()))).Union(
                             model.AttachmentStreams.Select((x, index) => (stream: (FfmpegStream)x, index, type: 3, list: model.AttachmentStreams.Select(x => (FfmpegStream)x).ToList()))))
        {
            ...
            VideoFileStream vfs = item.stream is FfmpegVideoStream ? ((FfmpegVideoStream)item.stream).Stream :
                ...
                item.stream is FfmpegSubtitleStream ? ((FfmpegSubtitleStream)item.stream).Stream : 
                ((FfmpegAttachmentStream)item.stream).Stream;
